...
def entrenar_kmeans():
    ...
    ruta_guardar = filedialog.askdirectory(title="Ruta para guardar MODELO...")
    ...
    # coge solo jpg o tif
    onlyfiles = [f for f in os.listdir(dir_path) if os.path.isfile("\\".join([dir_path, f])) and ".jpg" in f or ".tif" in f ]
    ...
    km = KMeans( n_clusters=4 ) # 3 clasificaciones
...
def predecir_cl():
    ...
    # Crea carpeta de resultados general
    ruta_guardar = filedialog.askdirectory(title="Nombre del directorio para guardar las fotos")
    #ruta_guardar = "C:\\Users\\Usuario\\Desktop\\Fotos para contar\\output_segundo_modelo"
    try:
        os.mkdir(ruta_guardar) #
    except:
            ...
            for filename in files:
                ...
                if filename:
                    ...
                    for i in range(df_freq.shape[0]):
                        ...
                        if df_freq.iloc[i, iloclabel] == 3:
                            n_circ_c4.append(df_freq.iloc[i, ilocount])
                    ...
                    df_freq["nº circulos c4"] = n_circ_c4
                ...
                for row_info_circle in df_tmp.values.tolist():
                    ...
                    if label_col == 0:
                        ...
                        cv2.circle(image, (x, y), r, (255, 0, 0), 2)  # Dibujar círculos azules en la imagen
                ...
                contador_c4 = df_freq[df_freq["filename"] == filename][["nº circulos c4"]].to_string()
                ...
                cv2.putText(image, "Cluster 4 - B : "+str(contador_c4), (20, 150) , cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2)
...
ventana.title("CALCULAR VIABILIDAD para CIBMS-CSIC")
...
menu_archivo.add_command(label="Calcular viabilidad (Thunder)", command=predecir_cl)
